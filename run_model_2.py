...
def learner(epochs,timeall,trajectories):
    ...
    for epoch in range(epochs):
        ...
        for trajectory in range(trajectories):
            ...
            nsample = 40
            itersample = random.sample(range(numbreaks),nsample)
            save_error= 0
            for iter in itersample:
                #old = make_img(trajectory,int(400/numbreaks * iter),dataset_main)
                ...
                #targ_img = make_img(trajectory, int(400/numbreaks * (iter + 1)),dataset_main)
                targ_img = make_img(trajectory, iter + 1,dataset_main)
                #for time in range(int(400/numbreaks * iter + 1),int(400/numbreaks * (iter + 1)) + 1):
                
                new = model(old.to(device))
                old = new                  
                
                new = new.cpu()
                time = Timeall
                error = loss_fn(save_old,new,targ_img,time,trajectory,save_loss)
                error = torch.mean(error)
                save_error=error+save_error
            loss = torch.mean(save_error)
            loss.backward()
            print('Epoch loss', loss)
            optimzer.step()
